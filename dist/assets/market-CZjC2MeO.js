import{i as v,f as g,a as u,b as p,g as $}from"./local-storage-Bd1VrlsT.js";import{c as y,s as L,u as w}from"./navigation-UNvGwtiV.js";async function E(){if(!await y()){window.location.href="/src/pages/auth.html";return}v(),L(),w()}E();document.addEventListener("DOMContentLoaded",async()=>{let l=null;const n=new URLSearchParams(window.location.search).get("channel");async function f(){l=await $();const t=document.getElementById("currentPoints");l&&t&&(t.textContent=l.points.toLocaleString())}async function c(t){const e=document.getElementById("productGrid");e&&(e.innerHTML=t.map(a=>`
        ...
        `).join(""),e.querySelectorAll(".product-card").forEach(a=>{a.addEventListener("click",s=>{if(s.target.classList.contains("favorite-btn")){s.preventDefault();return}const i=a.dataset.id;window.location.href=`/src/pages/product-detail.html?id=${i}`})}))}async function h(){const t=await u(),e=document.querySelector(".channel-list"),a=document.getElementById("marketHome");!e||!a||(n?a.style.display="none":(a.style.display="block",e.innerHTML=t.map(s=>`
            ...
            `).join("")))}const o=await g();let d=o;if(n){d=o.filter(a=>a.channel_id===n);const e=(await u()).find(a=>a.id===n);if(e){const a=document.querySelector(".market-container"),s=document.createElement("div");s.className="channel-header-large",s.innerHTML=`
